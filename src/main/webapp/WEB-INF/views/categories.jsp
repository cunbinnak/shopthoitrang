							...
							<table class="table table-borderless table-data3" id="myTable">
								...
								<tbody>
									<c:forEach items="${listCate}" var="cate">
										<tr>
											<td>${cate.catalogId}</td>
											<td>${cate.catalogName}</td>
											<td>${cate.catalogDes}</td>
											<td class="process"><c:choose>
													<c:when test="${cate.catalogStatus}">Active</c:when>
													<c:otherwise>Inactive</c:otherwise>
												</c:choose></td>
											<td>${cate.parentId}</td>
											<td>
												<div class="table-data-feature">
													 <!-- <a href="viewcatedetails?id=${cate.catalogId}"
														class="item" data-toggle="tooltip" data-placement="top"
														title="View Details"> <i class="fa fa-eye"
														aria-hidden="true"></i>
													</a> -->
												 <a href="initeditcate?id=${cate.catalogId}" class="item"
														data-toggle="tooltip" data-placement="top" title="Edit">
														<i class="zmdi zmdi-edit"></i>
													</a> 
													<a href="deletecate?id=${cate.catalogId}" class="item"
														data-toggle="tooltip" data-placement="top" title="Delete" onclick="return confirm('Are You Sure?')">
